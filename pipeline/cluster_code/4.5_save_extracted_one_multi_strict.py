...
dir = '/media/c/D/zzw-c4/cluster_data'
...
def extract_one_cluster_data(topk_v, topk_i, device, threshold=0.75, min_community_size=10):
    ...
    for i in range(len(topk_v)):
        ...
        for j in range(topk_v.shape[1]):
            if topk_v[i][j] >= threshold:
                ...
                if topk_i_list[j] not in new_cluster:
